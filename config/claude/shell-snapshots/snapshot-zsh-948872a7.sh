...
detect-clipboard () {
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | pbcopy
		}
		clippaste () {
			pbpaste
		}
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" > /dev/clipboard
		}
		clippaste () {
			bat --paging=never /dev/clipboard
		}
	elif (( $+commands[clip.exe] )) && (( $+commands[powershell.exe] ))
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | clip.exe
		}
		clippaste () {
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | wl-copy &> /dev/null &|
		}
		clippaste () {
			wl-paste --no-newline
		}
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | xsel --clipboard --input
		}
		clippaste () {
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | xclip -selection clipboard -in &> /dev/null &|
		}
		clippaste () {
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | lemonade copy
		}
		clippaste () {
			lemonade paste
		}
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | doitclient wclip
		}
		clippaste () {
			doitclient wclip -r
		}
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | win32yank -i
		}
		clippaste () {
			win32yank -o
		}
	...
	then
		clipcopy () {
			bat --paging=never "${1:-/dev/stdin}" | termux-clipboard-set
		}
		clippaste () {
...
handle_completion_insecurities () {
	...
	eza --color=always --icons -ld "${(@)insecure_dirs}"
	bat --paging=never <<EOD
...
ws () {
	local proj="$1" 
	[[ -z "$proj" ]] && {
		echo "usage: ws <project>"
		return 1
	}
	local dir
	dir="$(zoxide query "$proj" 2>/dev/null || echo "$proj")" 
	cd "$dir" || {
		echo "ws: cannot cd into $dir"
		return 1
	}
...
zsh_stats () {
	fc -l 1 | awk '{ CMD[$2]++; count++; } END { for (a in CMD) print CMD[a] " " CMD[a]*100/count "% " a }' | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -v "./" | sort -nr | head -n 20 | column -c3 -s " " -t | nl
}
zshz () {
...
if ! command -v rg >/dev/null 2>&1; then
  alias rg='/Users/nathanvale/.nvm/versions/node/v20.18.0/lib/node_modules/@anthropic-ai/claude-code/vendor/ripgrep/arm64-darwin/rg'
fi
export PATH='/Users/nathanvale/.console-ninja/.bin:/Users/nathanvale/code/dotfiles/genie/scripts:/Users/nathanvale/.nvm/versions/node/v20.18.0/bin:/Users/nathanvale/code/dotfiles/genie/bin:/Users/nathanvale/.local/share/pnpm/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/bin:/bin:/Users/nathanvale/.console-ninja/.bin:/Users/nathanvale/code/dotfiles/genie/scripts:/Users/nathanvale/.nvm/versions/node/v20.18.0/bin:/Users/nathanvale/code/dotfiles/genie/bin:/Users/nathanvale/.local/share/pnpm/bin:/System/Cryptexes/App/usr/bin:/usr/sbin:/sbin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin:/Users/nathanvale/code/dotfiles/bin:/Applications/Ghostty.app/Contents/MacOS:/Users/nathanvale/Library/pnpm:/opt/homebrew/opt/fzf/bin'
